...
`).map(t=>t.trim());console.log(i),console.log(c);let a=[];for(const t of i){let d=t.split("	")[2].split(" ");d=d.filter(s=>!s.endsWith(".")),d.every((s,N)=>s===c[N])&&a.push({t:d,m:t.split("	")[1],a:t.split("	")[0][0]})}return console.log(a),a=={}?null:(a.sort(t=>{t.t.length}),a[a.length-1].a==["A"]&&(a[a.length-1].a=["Flank Attack"]),a[a.length-1].a==["B"]&&(a[a.length-1].a=["Semi-Open Game"]),a[a.length-1].a==["C"]&&(a[a.length-1].a=["Open Game"]),a[a.length-1].a==["D"]&&(a[a.length-1].a=["Closed Position"]),a[a.length-1].a==["E"]&&(a[a.length-1].a=["Indian Game"]),a[a.length-1])}const ka={created(){try{this.pgn=(re().query.pgn??void 0).toString(),console.log(this.pgn),this.chess.loadPgn(this.pgn),this.history=this.chess.history().reverse(),this.chess.reset();const c=t=>{const d=/\[BlackElo "(.*?)"]/.exec(t);return d&&d.length>=2?d[1]:null};this.blackElo=c(this.pgn);const e=t=>{const d=/\[WhiteElo "(.*?)"]/.exec(t);return d&&d.length>=2?d[1]:null};this.whiteElo=e(this.pgn);const i=t=>{const d=/\[White "(.*?)"]/.exec(t);return d&&d.length>=2?d[1]:null};this.whitePlayer=i(this.pgn);const a=t=>{const d=/\[Black "(.*?)"]/.exec(t);return d&&d.length>=2?d[1]:null};this.blackPlayer=a(this.pgn)}catch{this.chess=null}},data(){return{fen:"",chess:new le,score:0,stockfish:null,custom:0,size:600,history:null,pgn:"",blackElo:"",whiteElo:"",blackPlayer:"",whitePlayer:"",stockfishWorking:!1,opening:{m:"Startin Position",t:[],a:[]},bestmove:""}},methods:{evaluatePosition(){this.opening=Ga(this.chess.history()),be(this.fen,(c,e)=>{console.log("Received score:"+c),console.log("Received Bestmove:"+e),this.score=c,(this.bestmove==""||this.bestmove.length<=e.length||this.bestmove.split(" ")[0]!=e.split(" ")[0])&&(this.bestmove=e)}),setTimeout(()=>{this.stockfishWorking=!1},1e3)},onMovePlayed({move:c,game:e}){e.makeMove(c),console.log("FEEEEEEEEEEEEEEEEEEN:"+e.fen),this.fen=e.fen,this.evaluatePosition()},moveCall(){console.log("moveCall"),!this.chess.isGameOver()&&!this.stockfishWorking&&(this.stockfishWorking=!0,console.log("Next Move load:"),this.chess.move(this.history.pop()),this.fen=this.chess.fen(),this.evaluatePosition())}},components:{EvalBar:oa,ChessBoard:ca,UserAnalyzeBar:Da,EvalCircle:pa}},Aa={key:0,class:"grid place-content-center min-h-screen text-secondary font-mono"},Sa={key:1,class:"flex items-center overflow-hidden h-screen w-full flex-row py-8"},ya=o("div",{class:"basis-1/12 h-full"},null,-1),Qa={class:"flex flex-row h-full items-center basis-2/3 w-full"},Ka={class:"h-full flex flex-col"},va={class:"bg-base-300 w-full h-full basis-1/12 py-2 px-2 border-solid border-secondary border-2 my-2 overflow-hidden rounded-lg"},wa={class:"bg-base-300 w-full h-full basis-1/12 py-2 px-2 border-solid border-secondary border-2 my-2 overflow-hidden rounded-lg"},Ma={class:"mx-4 h-full w-fit border-2 border-secondary flex flex-col items-center justify-center bg-base-200 rounded-lg"},za={class:"grid grid-rows-3 w-full h-full place-items-center"},Pa=o("div",{class:"w-16 mx-4 h-0 border border-secondary"},null,-1),La={class:"grid grid-rows-3 h-full w-full place-items-center"},Ra={class:"h-full w-full bg-base-100 m-2 flex flex-col"},Ea=o("div",{class:"w-full h-1/3 p-2"},[o("div",{class:"bg-primary border-8 rounded-lg h-full w-full"})],-1),Ia={class:"w-full h-1/4 p-2"},Fa={class:"bg-base-300 rounded-lg h-full w-full p-2"},Ta=o("br",null,null,-1),_a={class:"badge badge-accent font-bold mr-2"},Ha=o("div",{class:"w-full h-1/2 p-2"},[o("div",{class:"bg-base-300 rounded-lg h-full w-full"})],-1),Wa={class:"bottom-3 w-full h-12 grid grid-cols-5 gap-x-1"},ja=p('<button class="btn-accent btn"><span class="material-symbols-outlined"> keyboard_double_arrow_left </span></button><button class="btn-accent btn"><span class="material-symbols-outlined"> chevron_left </span></button><button class="btn-accent btn"><span class="material-symbols-outlined"> auto_awesome </span></button>',3),Za={class:"btn-accent btn"},$a=o("button",{class:"btn-accent btn"},[o("span",{class:"material-symbols-outlined"}," keyboard_double_arrow_right ")],-1);function qa(c,e,i,a,t,n){const d=x("EvalBar"),s=x("UserAnalyzeBar"),N=x("chess-board"),b=x("EvalCircle");return this.chess===null?(f(),l("div",Aa," Invalide PGN ")):(f(),l("div",Sa,[ya,o("div",Qa,[u(d,{ref:"evalBar",evaluation:t.score,class:"h-full"},null,8,["evaluation"]),o("div",Ka,[o("div",va,[u(s,{color:!1,elo:this.blackElo,username:this.blackPlayer},null,8,["elo","username"])]),u(N,{onOnMovePlayed:n.onMovePlayed,fen:t.fen,"onUpdate:fen":e[0]||(e[0]=G=>t.fen=G),size:520},null,8,["onOnMovePlayed","fen"]),o("div",wa,[u(s,{color:!0,elo:this.whiteElo,username:this.whitePlayer},null,8,["elo","username"])])]),o("div",Ma,[o("div",za,[u(b,{evaluation:90}),u(b,{evaluation:20}),u(b,{evaluation:40})]),Pa,o("div",La,[u(b,{evaluation:90}),u(b,{evaluation:20}),u(b,{evaluation:40})])]),o("div",Ra,[Ea,o("div",Ia,[o("div",Fa,[Z(y(t.opening.m)+" ",1),Ta,(f(!0),l(Q,null,K(t.opening.a,(G,F)=>(f(),l("div",{key:F},[o("div",_a,y(G),1)]))),128)),Z(" "+y(t.bestmove),1)])]),Ha,o("div",Wa,[ja,o("button",Za,[o("span",{class:"material-symbols-outlined",onClick:e[1]||(e[1]=(...G)=>n.moveCall&&n.moveCall(...G))}," chevron_right ")]),$a])])])]))}const Xa=I(ka,[["render",qa]]);export{Xa as default};
