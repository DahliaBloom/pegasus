...
`).map(t=>t.trim());console.log(i),console.log(c);let n=[];for(const t of i){let N=t.split("	")[2].split(" ");N=N.filter(d=>!d.endsWith(".")),N.every((d,s)=>d===c[s])&&n.push({t:N,m:t.split("	")[1]})}return console.log(n),n=={}?null:(n.sort(t=>{t.t.length}),n[n.length-1].m)}const Ga={created(){try{this.pgn=(oe().query.pgn??void 0).toString(),console.log(this.pgn),this.chess.loadPgn(this.pgn),this.history=this.chess.history().reverse(),this.chess.reset();const c=t=>{const N=/\[BlackElo "(.*?)"]/.exec(t);return N&&N.length>=2?N[1]:null};this.blackElo=c(this.pgn);const e=t=>{const N=/\[WhiteElo "(.*?)"]/.exec(t);return N&&N.length>=2?N[1]:null};this.whiteElo=e(this.pgn);const i=t=>{const N=/\[White "(.*?)"]/.exec(t);return N&&N.length>=2?N[1]:null};this.whitePlayer=i(this.pgn);const n=t=>{const N=/\[Black "(.*?)"]/.exec(t);return N&&N.length>=2?N[1]:null};this.blackPlayer=n(this.pgn)}catch{this.chess=null}},data(){return{fen:"",chess:new Be,score:0,stockfish:null,custom:0,size:600,history:null,pgn:"",blackElo:"",whiteElo:"",blackPlayer:"",whitePlayer:"",stockfishWorking:!1,opening:"Startin Position"}},methods:{evaluatePosition(){this.opening=pa(this.chess.history()),ge(this.fen,c=>{console.log("Received score:",c),this.score=c}),setTimeout(()=>{this.stockfishWorking=!1},1e3)},onMovePlayed({move:c,game:e}){e.makeMove(c),console.log("FEEEEEEEEEEEEEEEEEEN:"+e.fen),this.fen=e.fen,this.evaluatePosition()},moveCall(){console.log("moveCall"),!this.chess.isGameOver()&&!this.stockfishWorking&&(this.stockfishWorking=!0,console.log("Next Move load:"),this.chess.move(this.history.pop()),this.fen=this.chess.fen(),this.evaluatePosition())}},components:{EvalBar:fa,ChessBoard:na,UserAnalyzeBar:ma,EvalCircle:Ca}},Aa={key:0,class:"grid place-content-center min-h-screen text-secondary font-mono"},ka={key:1,class:"flex items-center overflow-hidden h-screen w-full flex-row py-8"},Sa=o("div",{class:"basis-1/12 h-full"},null,-1),ya={class:"flex flex-row h-full items-center basis-2/3 w-full"},Qa={class:"h-full flex flex-col"},Ka={class:"bg-base-300 w-full h-full basis-1/12 py-2 px-2 border-solid border-secondary border-2 my-2 overflow-hidden rounded-lg"},va={class:"bg-base-300 w-full h-full basis-1/12 py-2 px-2 border-solid border-secondary border-2 my-2 overflow-hidden rounded-lg"},wa={class:"mx-4 h-full w-fit border-2 border-secondary flex flex-col items-center justify-center bg-base-200 rounded-lg"},za={class:"grid grid-rows-3 w-full h-full place-items-center"},Ma=o("div",{class:"w-16 mx-4 h-0 border border-secondary"},null,-1),Pa={class:"grid grid-rows-3 h-full w-full place-items-center"},La={class:"h-full w-full bg-base-100 m-2 flex flex-col"},Ra=o("div",{class:"w-full h-1/3 p-2"},[o("div",{class:"bg-primary border-8 rounded-lg h-full w-full"})],-1),Ea={class:"w-full h-1/4 p-2"},Ia={class:"bg-base-300 rounded-lg h-full w-full p-2"},Fa=o("div",{class:"w-full h-1/2 p-2"},[o("div",{class:"bg-base-300 rounded-lg h-full w-full"})],-1),Ta={class:"bottom-3 w-full h-12 grid grid-cols-5 gap-x-1"},_a=p('<button class="btn-accent btn"><span class="material-symbols-outlined"> keyboard_double_arrow_left </span></button><button class="btn-accent btn"><span class="material-symbols-outlined"> chevron_left </span></button><button class="btn-accent btn"><span class="material-symbols-outlined"> auto_awesome </span></button>',3),Ha={class:"btn-accent btn"},Wa=o("button",{class:"btn-accent btn"},[o("span",{class:"material-symbols-outlined"}," keyboard_double_arrow_right ")],-1);function ja(c,e,i,n,t,a){const N=x("EvalBar"),d=x("UserAnalyzeBar"),s=x("chess-board"),b=x("EvalCircle");return this.chess===null?(f(),g("div",Aa," Invalide PGN ")):(f(),g("div",ka,[Sa,o("div",ya,[u(N,{ref:"evalBar",evaluation:t.score,class:"h-full"},null,8,["evaluation"]),o("div",Qa,[o("div",Ka,[u(d,{color:!1,elo:this.blackElo,username:this.blackPlayer},null,8,["elo","username"])]),u(s,{onOnMovePlayed:a.onMovePlayed,fen:t.fen,"onUpdate:fen":e[0]||(e[0]=S=>t.fen=S),size:520},null,8,["onOnMovePlayed","fen"]),o("div",va,[u(d,{color:!0,elo:this.whiteElo,username:this.whitePlayer},null,8,["elo","username"])])]),o("div",wa,[o("div",za,[u(b,{evaluation:90}),u(b,{evaluation:20}),u(b,{evaluation:40})]),Ma,o("div",Pa,[u(b,{evaluation:90}),u(b,{evaluation:20}),u(b,{evaluation:40})])]),o("div",La,[Ra,o("div",Ea,[o("div",Ia,Q(t.opening),1)]),Fa,o("div",Ta,[_a,o("button",Ha,[o("span",{class:"material-symbols-outlined",onClick:e[1]||(e[1]=(...S)=>a.moveCall&&a.moveCall(...S))}," chevron_right ")]),Wa])])])]))}const Ya=I(Ga,[["render",ja]]);export{Ya as default};
